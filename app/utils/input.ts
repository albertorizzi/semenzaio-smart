...
export function validateCrop(crop: ICrop): { ok: boolean, error?: string } {
    ...
    if (crop.setting.lightHours.fiorituraPhase < 0 || crop.setting.lightHours.fiorituraPhase > 24) {
        return { ok: false, error: 'Lighthours must be between 0 and 24' };
    }

    if (crop.setting.lightHours.vegetativaPhase < 0 || crop.setting.lightHours.vegetativaPhase > 24) {
        return { ok: false, error: 'Lighthours must be between 0 and 24' };
    }
